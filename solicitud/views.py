...
@login_required()
@transaction.atomic
def solicitud_1(request, codigo):
	error = ''#True
	form = SolicitudFrm()
	...
	if request.POST:
		with transaction.atomic():
			try:
				...
				compa = request.POST.get('compania')
				ingreso = Solicitud.objects.get(codsolicitud=codigo)
				ingreso.cod_user= request.user
				...
				return redirect(reverse('solicitud_2', kwargs={'codigo': ingreso.codsolicitud}))
	...
	return render(request, 'solicitud_1.html', ctx)
...
def solicitud_2(request, codigo):
	...
	if request.POST:
		with transaction.atomic():
			try:
				...
				ingreso.vista = 3
				ingreso.save()
				return redirect(reverse('solicitud_3', kwargs={'codigo': ingreso.codsolicitud}))
	...
	return render(request, 'solicitud_2.html', ctx )
...
def solicitud_3(request, codigo):
		...
		with transaction.atomic():
			try:
				...
				for x in tipo_cuenta:
					...
					cuenta.cantidad = 0 if cantidad[contcuentas] == '' else cantidad[contcuentas]
					cuenta.institucion = None if institucion[contcuentas] == '' else institucion[contcuentas]
					cuenta.numero_cuenta = 'No ingresada' if numero_cuenta[contcuentas] == '' else numero_cuenta[contcuentas]
					cuenta.save()
					contcuentas += 1
				...
				ingreso.nombrequienrecomienda = 'Ninguna' if request.POST.get('nombrequienrecomienda') == '' else request.POST.get('nombrequienrecomienda')
				...
				ingreso.vista = 4
				ingreso.save()
				return redirect(reverse('solicitud_4', kwargs={'codigo': ingreso.codsolicitud}))
			except Exception as e:
				print 'error', e
				error = True
	...
	return render(request, 'solicitud_3.html', ctx )
...
def solicitud_4(request, codigo): 
	...
	if request.POST:
		with transaction.atomic(): 
			try:
				...
				salariofinal = request.POST.getlist('salariofinal[]')
				puesto = request.POST.getlist('puesto[]')
				fucionespuesto = request.POST.getlist('fucionespuesto[]')
				...
				for x in nombreempresa:
					...
					if bool(motivoretiro[counter]):
						laboral.fechaegreso = fechaegreso[counter]
					else:
						laboral.fechaegreso = None
					...
					laboral.salariofinal = None if salariofinal[counter] == '' else salariofinal[counter]
					laboral.puesto = None if puesto[counter] == '' else puesto[counter]
					laboral.fucionespuesto = None if fucionespuesto[counter] == '' else fucionespuesto[counter]
					laboral.save()
					counter += 1

				return redirect(reverse('solicitud_5', kwargs={'codigo': ingreso.codsolicitud }))
			except Exception as e:
				error = True
				print 'error',e
	ctx = {
	...
	return render(request, 'solicitud_4.html', ctx )
...
def solicitud_5(request, codigo):
	...
	if request.POST:
		with transaction.atomic():
			try:
				...
				ingreso.vista = 6
				ingreso.save()
				return redirect(reverse('solicitud_6', kwargs={'codigo': ingreso.codsolicitud }))
	...
	return render(request, 'solicitud_5.html', ctx )
 
@login_required()
@transaction.atomic
def solicitud_6(request, codigo):
	solicitud = Solicitud.objects.get(codsolicitud=codigo)
	sol = SolicitudFrm()
	error = ''
	form = ReferenciaFrm()
	...
	if request.POST:
		tiene_lab = request.POST.get('chckNo_aplica')
		with transaction.atomic():
			try:
				if tiene_lab is None:
					nombrecompleto = request.POST.getlist('nombrecompleto[]')
				...
				solicitud = Solicitud.objects.get(codsolicitud=codigo)
				solicitud.observacionesadicionales = request.POST.get('observaciones_adicionales')
				try:
					request.FILES['cv'].name = force_to_unicode(request.FILES['cv'].name)
					archivo = request.FILES['cv']
					solicitud.cv = archivo
					solicitud.cvnombre = archivo.name
...
@login_required()
@transaction.atomic
def no_historial_laboral(request):
	cod_solicitud = Solicitud.objects.get(cod_user=request.user.id)
	Solicitud.objects.filter(codsolicitud=cod_solicitud.codsolicitud).update(vista = 5)
	return redirect(reverse('solicitud_5', kwargs={'codigo':cod_solicitud.codsolicitud }))
